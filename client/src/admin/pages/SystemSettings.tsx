...
interface GoogleOAuthUpdateResponse {
    success: boolean;
    message: string;
    data: GoogleOAuthConfig;
}
...
// Google OAuth Settings Component
function GoogleOAuthSettings() {
    ...
    // Toggle Google Auth enabled/disabled
    const handleToggleGoogleAuth = async (enabled: boolean) => {
        setIsSavingEnabled(true);
        try {
            const adminToken = localStorage.getItem('admin-token') || sessionStorage.getItem('admin-token');
            // Use the existing endpoint PUT /api/admin/settings/:key
            const res = await fetch('/api/admin/settings/google_oauth.enabled', {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${adminToken}`,
                    'Content-Type': 'application/json',
            ...
            toast({
                title: "Success",
                description: `Google OAuth is now ${enabled ? 'active' : 'non-active'}`,
            });
            queryClient.invalidateQueries({ queryKey: ["/api/admin/settings/google-oauth"] });
        } catch (error) {
            toast({
                title: "Error",
                description: "Failed to update Google OAuth status",
                variant: "destructive",
            });
            setGoogleAuthEnabled(!enabled);
        } finally {
    ...
    const { data, isLoading, error } = useQuery<GoogleOAuthResponse>({
    ...
    // Sync googleAuthEnabled state with fetched data
    useEffect(() => {
        if (data?.data?.enabled !== undefined) {
            setGoogleAuthEnabled(data.data.enabled);
        }
    }, [data]);
    ...
    const updateConfigMutation = useMutation({
        mutationFn: async (config: typeof formData) => {
            ...
            if (!res.ok) {
                const error = await res.json();
                throw new Error(error.error?.message || 'Failed to update Google OAuth settings');
            }

            return res.json() as Promise<GoogleOAuthUpdateResponse>;
        },
        onSuccess: (response) => {
            toast({
                title: "Settings Updated",
                description: response.message || "Google OAuth settings have been updated successfully.",
            });
            queryClient.invalidateQueries({ queryKey: ["/api/admin/settings/google-oauth"] });
            setIsEditing(false);
        },
    ...
    const testConfigMutation = useMutation({
        mutationFn: async () => {
            ...
            if (!res.ok) {
                const error = await res.json();
                throw new Error(error.error?.message || 'Failed to test Google OAuth settings');
            }

            return res.json() as Promise<GoogleOAuthTestResponse>;
        },
        onSuccess: (response) => {
            setTestResult({
    ...
    const handleEdit = () => {
        if (data?.data) {
            setFormData({
                ...
                callbackUrl: data.data.callbackUrl || window.location.origin + '/api/auth/google/callback',
            });
        } else {
            setFormData({
                clientId: '',
    ...
    const handleSave = () => {
        ...
        try {
            ...
            toast({
                title: "Validation Error",
                description: "Callback URL must be a valid URL.",
                variant: "destructive",
            });
            return;
        }
    ...
    // Check if Google OAuth is actually configured (from API response or fallback to local check)
    const isConfigured = config?.isConfigured ?? (config?.hasClientId && config?.hasClientSecret);
            ...
            <Card className="border-slate-200">
                <CardContent className="pt-6">
                    <Alert variant="destructive">
                        <AlertCircle className="h-4 w-4" />
                        <AlertDescription>
                            Failed to load Google OAuth settings. Please try again.
                        </AlertDescription>
                    </Alert>
                </CardContent>
            </Card>
        ...
        <Card className="border-slate-200">
            <CardHeader className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <CardDescription>
                            Configure Google Sign-In for user authentication
                        </CardDescription>
                    </div>
                    {!isEditing && (
                        <Button
            ...
            <CardContent className="pt-6">
                {isEditing ? (
                    <div className="space-y-6">
                        ...
                        <Alert className="bg-blue-50 border-blue-200">
                            ...
                            <AlertDescription className="text-blue-900">
                                ...
                                . Make sure to add the callback URL to your OAuth 2.0 Client's authorized redirect URIs.
                            </AlertDescription>
                        </Alert>
                        ...
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-slate-500">
                                The URL Google will redirect to after authentication. Add this to your Google Cloud Console.
                            </p>
                        </div>
                    ...
                    <div className="space-y-6">
                        {/* Enable/Disable Google Auth Toggle */}
                        <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border-2 border-blue-200">
                            <div className="flex-1">
                                ...
                                <div className="text-sm text-blue-700 mt-1">
                                    {googleAuthEnabled
                                        ? "Google OAuth is currently active - users can sign in with Google"
                                        : "Google OAuth is currently non-active - users cannot sign in with Google"}
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <Badge
                                    variant={googleAuthEnabled ? "default" : "secondary"}
                                    className={googleAuthEnabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}
                                >
                                    {googleAuthEnabled ? "Active" : "Non-Active"}
                                </Badge>
                                <Switch
                                    checked={googleAuthEnabled}
                                    onCheckedChange={handleToggleGoogleAuth}
                                    disabled={isSavingEnabled || !isConfigured}
                        ...
                        {!isConfigured && (
                            <Alert className="bg-amber-50 border-amber-200">
                                ...
                                <AlertDescription className="text-amber-900">
                                    Google OAuth is not configured. Please set up Client ID and Client Secret below to activate this feature.
                                </AlertDescription>
                            </Alert>
                        )}
                        ...
                        {testResult && (
                            ...
                            >
                                ...
                                <AlertDescription className={testResult.success ? "text-green-900" : ""}>
                                    {testResult.message}
                                </AlertDescription>
                            </Alert>
                        )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                                <div className="flex-1">
                                    <div className="font-medium text-slate-900">Configuration Status</div>
                                    <div className="text-sm text-slate-500 mt-1">
                                ...
                                <Badge
                                    ...
                                    className={isConfigured ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}
                                >
                            ...
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                                <div className="flex-1">
                                    <div className="font-medium text-slate-900">Client ID</div>
                                    <div className="text-sm text-slate-500 mt-1">
                                ...
                                <div className="flex items-center gap-3">
                                    {config?.hasClientId ? (
                                        <>
                                            ...
                                            <div className="text-sm text-slate-600 font-mono bg-white px-3 py-1 rounded border border-slate-200 max-w-[200px] truncate">
                                                {config.clientId}
                                            </div>
                                        </>
                                    ) : (
                            ...
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                                <div className="flex-1">
                                    <div className="font-medium text-slate-900">Client Secret</div>
                                    <div className="text-sm text-slate-500 mt-1">
                            ...
                            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                                <div className="flex-1">
                                    <div className="font-medium text-slate-900">Callback URL</div>
                                    <div className="text-sm text-slate-500 mt-1">
                                        OAuth redirect URI
                                    </div>
                                </div>
                                <div className="text-sm text-slate-600 font-mono bg-white px-3 py-1 rounded border border-slate-200 max-w-[300px] truncate">
                                    {config?.callbackUrl || 'Not configured'}
                                </div>
                            </div>
                        ...
                        <div className="pt-6 border-t border-slate-200">
                            ...
                            <div className="flex items-center gap-3">
                                <Button
                                    ...
                                    disabled={testConfigMutation.isPending || !config?.enabled}
                                    variant="outline"
                                    className="border-blue-200 text-blue-600 hover:bg-blue-50"
                                >
                                ...
                                <p className="text-sm text-slate-500">
                                    Validates the format of your Google OAuth credentials
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <Card className="border-slate-200 shadow-md">
                    <CardContent className="p-6">
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                            ...
                            <TabsContent value="google-oauth" className="mt-6">
                                <GoogleOAuthSettings />
                            </TabsContent>
